...
public class spring_sys extends PApplet {
...
float radius = 0.01f; // radius of oscillating mass in meters
...
float spiralConstant = 1 / 10.0f; // number of spirals on the spring per pixel
...
// Mass position
  ...
  pos_initial = new PVector(width / 2.0f, height / 2.0f);
...
// Springs
  ...
  {
    springs.add(new Spring(width/4.0f, height/4.0f,
      abs(length(width/4.0f, height/4.0f) - pos_now.mag())/4.0f, 20, 3, 1, true));
        
    springs.add(new Spring(width/2.0f, 3.0f*height/4.0f,
      abs(length(3.0f*width/4.0f, 3.0f*height/4.0f) - pos_now.mag())/4.0f, 20, 3, 2, true));
...
public void checkGUIChanges()
{
  if(massSelected)
  {
    ...
    float parsedDensity = PApplet.parseFloat(textDensity);
    ...
    float parsedRadius = PApplet.parseFloat(textRadius);
  ...
  if(anySpringSelected && lastSelectedSpringPosition >= 0)
  {
    ...
    float parsedK = PApplet.parseFloat(textK);
    float parsedL = PApplet.parseFloat(textL);
    float parsedLambda = PApplet.parseFloat(textLambda);
...
class Spring
{
  ...
  {
    this.L = L;
    spiralNumber = PApplet.parseInt(L * spiralConstant);
  }
...
{
  mass = 4.0f/3.0f * pow(radius,3) * PI * density;
}
...
public float length(float x, float y)
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "spring_sys" };
    if (passedArgs != null) {
